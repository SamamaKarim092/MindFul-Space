...
export default function InteractivePrompts() {
  ...
  useEffect(() => {
    const handleMoodUpdate = (e: CustomEvent<{ mood?: string }>) => {      const newMood = e.detail.mood?.toUpperCase() || "NEUTRAL";
  ...
  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 relative overflow-hidden group">
      <div className="absolute top-0 right-0 p-4 opacity-20 group-hover:opacity-100 transition-opacity">
        <Sparkles className={`w-5 h-5 transition-colors duration-500 ${currentMood.accent}`} />
      </div>

      <div className="flex flex-col gap-2">
        <span className={`text-[10px] font-bold uppercase tracking-[0.2em] transition-colors duration-500 ${currentMood.accent}`}>
          Guided Reflection
        </span>
      ...
      <div className="mt-4 flex gap-2">
        {activePrompts.map((_, i) => {
          ...
          const accentBg = accentToBg[currentMood.accent] || "bg-purple-400";
